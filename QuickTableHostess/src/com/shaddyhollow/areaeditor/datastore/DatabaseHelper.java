...
//			db.execSQL("CREATE TABLE " + T_CARRYOUTS + "(_id integer primary key autoincrement, uuid text, visit_uuid text, name text, phone_number text, status text, cartitems text, comments text, created_at string, removed integer default 0);");
		...
		if(oldVersion<29) {
			...
			db.execSQL("CREATE TABLE " + T_CARRYOUTS + "(_id integer primary key autoincrement, uuid text, visit_uuid text, name text, phone_number text, order_status text, patron_status text, cartitems text, comments text, created_at string, removed integer default 0);");
		}
		if(oldVersion<30) {
